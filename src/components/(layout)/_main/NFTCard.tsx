...
export default function NFTCard({ item }: { item: CollectionData }) {
  ...
  const { totalVolume, totalSales, isLoading: loadingVolumeSale, isError } = useVolumeAndSales(item.contract);
  return (
        ...
        <CardFooter className={cn("mt-2 flex w-full justify-between rounded-lg bg-muted p-3 dark:bg-muted/30")}>
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="flex flex-row truncate text-sm font-semibold">
              {loadingVolumeSale || !totalVolume ? <Loader2 className="animate-spin" size={14} /> : totalVolume.toFixed(2)} POL
            </p>
          </div>
        </CardFooter>
      </Card>
