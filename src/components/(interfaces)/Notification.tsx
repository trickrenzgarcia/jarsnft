  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button className='rounded-2xl w-[49px] h-[49px] p-2' variant='ghost'>
          <IoIosNotifications className='text-2xl' />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80">
        <div className='flex flex-col'>
          <div className='flex items-center justify-start p-4 border-b border-gray-200 dark:border-gray-600'>
            <h3 className='text-lg font-semibold'>Notifications</h3>
          </div>
          {loadingEvents && (<div className='flex items-center justify-center p-4'>
            <AiOutlineLoading className='animate-spin w-10 h-10' />
          </div>)}
          {events && auctionClosed && (
            <ScrollArea className='h-80 w-full flex flex-col p-4'>
