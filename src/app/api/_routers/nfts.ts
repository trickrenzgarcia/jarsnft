...
nfts.get("/views", async (c) => {
  ...
  const data = await db.query.nftViews.findFirst({
    where: (fields, operators) => operators.eq(fields.contract, schema.data.contractAddress) && operators.eq(fields.tokenId, schema.data.tokenId),
  });
...
nfts.post("/views", async (c) => {
  ...
  await db.transaction(async (tx) => {
    await db
      ...
      .where(and(eq(nftViews.id, data.id), eq(nftViews.contract, schema.data.contractAddress), eq(nftViews.tokenId, schema.data.tokenId)));
...
nfts.get("/tx", async (c) => {
  ...
  const provider = new ethers.providers.InfuraProvider(137, {
