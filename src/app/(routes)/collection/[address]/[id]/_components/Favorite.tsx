...
import toggleFavorite from "@/actions/toggleFavorites";
...
export default function Favorite({ favorite, address, id }: Props) {
  ...
  const [optimisticLikes, setOptimisticLikes] = useOptimistic(favorite.count, (state, amount: number) => state + Number(amount));
