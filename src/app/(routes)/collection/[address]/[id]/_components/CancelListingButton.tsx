...
export default function CancelListingButton({ listings, auctionListing }: CancelListingButtonProps) {
  ...
  const {
    ...
    error: errorCancelEnglishAuction,
  } = useCancelEnglishAuction(marketPlaceContract);

  const {
    mutateAsync: cancelDirectListing,
    isLoading: loadingCancelDirectListing,
    error: errorCancelDirectListing,
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <div className="flex items-center justify-between">
            ...
            <AlertDialogCancel className="rounded-full" disabled={cancelState != "idle"}>
              x
            </AlertDialogCancel>
          </div>
          <AlertDialogDescription>
