import { mysqlTable, mysqlSchema, AnyMySqlColumn, int, varchar, date, text, datetime, index, longtext, tinyint, unique, foreignKey } from "drizzle-orm/mysql-core"
...
export const nftEvents = mysqlTable("nft_events", {
	id: int("id").autoincrement().notNull(),
...
export const nonce = mysqlTable("nonce", {
	id: int("id").autoincrement().notNull(),
...
export const userProfile = mysqlTable("user_profile", {
	address: varchar("address", { length: 191 }).notNull().references(() => users.address, { onDelete: "restrict", onUpdate: "cascade" } ),
